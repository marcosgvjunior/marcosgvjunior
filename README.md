...
- **Advanced Analytics (PUC-Rio)**: TensorFlow and PyTorch foundations; computer vision applications including CNN-based classification/segmentation and industry use cases (face recognition, object tracking, liveness detection, document fraud detection, steganography, deepfakes, pose estimation) plus deployment considerations (network compression, embedded devices); NLP applications including text cleaning and preprocessing, tokenization, vector representations (one-hot, bag of words, TF-IDF, word embeddings), document similarity, sentiment analysis, and named entity recognition (completed).
